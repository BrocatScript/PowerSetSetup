...
try:
    ...
    logging.error(f"Не удалось импортировать модуль проверки обновлений: {e}")
...
logging.info(f"--- Запуск программы {Name_Program} {version} ---")
...
try:
    logging.debug("Запуск кодировки UTF-8")
    os.system('chcp 65001 > nul')
    logging.debug('Кодировщик UTF-8 применён!')
except Exception as e:
    logging.error('Не удалось применить кодировщик UTF-8')
    clear()
...
def cheack_os():
    logging.info('Проверка системы')
    if platform.system() == "Windows":
        logging.info('Система распознана как Windows')
        Main_Menu.main_menu()
    else:
        logging.error(f'Система распознана как {platform.system()}')
        error_os()
    ...
    try:
        if os.path.exists(config_path):
            with open(config_path, 'r', encoding='utf-8') as f:
                config = json.load(f)
                logging.info(f"Конфигурационный файл загружен: {config_path}")
                return config
        else:
            logging.warning(f"Конфигурационный файл не найден: {config_path}. Используются настройки по умолчанию.")
            os.makedirs(os.path.dirname(config_path), exist_ok=True)
            with open(config_path, 'w', encoding='utf-8') as f:
                json.dump(default_config, f, indent=4, ensure_ascii=False)
            logging.info(f"Создан конфигурационный файл с настройками по умолчанию: {config_path}")
...
def save_config(config):
    ...
    try:
        ...
        logging.info(f"Конфигурационный файл сохранен: {config_path}")
        return True
    except Exception as e:
...
def setup_localization(lang_code):
    ...
    try:
        ...
        logging.info(f"Язык установлен: {lang_code}")
    except Exception as e:
        logging.error(f"Ошибка при установке языка {lang_code}: {e}")
        _ = gettext.gettext

def apply_language_from_config():
    """Применяет язык из конфига с учетом auto_language"""
    config = read_config()
    
    if config.get("auto_language", True):
        # Если auto_language включен, определяем язык системы
        system_lang = get_system_lang()
        logging.info(f"Автоопределение языка: системный язык = {system_lang}")
        setup_localization(system_lang)
    else:
        # Если auto_language выключен, используем сохраненный язык
        saved_lang = config.get("language", "en")
        logging.info(f"Используется сохраненный язык: {saved_lang}")
...
def language_setting(new_lang):
    ...
    if save_config(config):
        logging.info("Язык сохранен в конфигурации")
...
def update_text_variables():
    ...
    global lang_disable_auto_language
    
    # errors
    global lang_error_os, lang_error_reset_scheme, lang_error_id_power_scheme, lang_error_timeout_20_minutes
    ...
    global lang_error_check_update, lang_error_input, lang_error_automode

    # logs
    global log_select_delete_sh, log_select_add_sh, log_select_activate_sh, log_language_changed
    global log_exit_program, log_progress, log_select_automode, log_activate_sh

    # logs errors
    global log_error_input, log_error_activate_sh, log_error_automode
    
    # Определение всех языковых строк
...
def get_powercfg_list():
    if platform.system() == 'Windows':
        try:
            ...
            logging.error(f"Ошибка при выполнении powercfg /l: {e}")
            return ""
    else:
        error_os()
        return ""
...
def powercfg_geta_sh():
    if platform.system() == 'Windows':
        try:
            ...
            logging.error(f"powercfg /getactivescheme: ({e})")
    else:
        error_os()
...
def run_auto_update_check():
    ...
    try:
        ...
        if not check_on_startup:
            logging.debug("Автоматическая проверка обновлений отключена")
            return False
        
        logging.info("Запуск автоматической проверки обновлений...")
        print(Fore.YELLOW + "\n" + "=" * 50)
        print(Fore.YELLOW + "АВТОМАТИЧЕСКАЯ ПРОВЕРКА ОБНОВЛЕНИЙ")
        print(Fore.YELLOW + "=" * 50 + Fore.RESET)
        
        # Загружаем конфиг через модуль обновлений (для правильной типизации)
        update_config = load_update_config()
        if not update_config:
            logging.error("Не удалось загрузить конфиг для проверки обновлений")
            return False
        ...
        if need_exit:
            logging.info("Обновление скачано и готово к установке. Завершение программы...")
            print(Fore.GREEN + "\nПрограмма будет закрыта для установки обновления.")
            print(Fore.GREEN + "Запустите программу снова после установки.")
            sleep(3)
            return True
        else:
            print(Fore.GREEN + "\nАвтоматическая проверка обновлений завершена.")
            print(Fore.GREEN + "Продолжаем работу программы...")
            sleep(2)
            return False
            
    except Exception as e:
        logging.error(f"Ошибка при автоматической проверке обновлений: {e}")
        print(Fore.RED + f"Ошибка проверки обновлений: {e}")
        sleep(2)
        return False

def manual_check_update():
    if not UPDATE_CHECKER_AVAILABLE:
        print(Fore.RED + "Модуль проверки обновлений недоступен")
        sleep(2)
        Main_Menu.main_menu()
        return
    ...
    try:
        update_config = load_update_config()
        if not update_config:
            print(Fore.RED + "Не удалось загрузить конфиг для проверки обновлений")
            sleep(2)
            Main_Menu.main_menu()
            return
        ...
        if need_exit:
            print(Fore.GREEN + "\nОбновление скачано и готово к установке. Завершение программы...")
            print(Fore.GREEN + "Запустите программу снова после установки.")
            sys.exit(3)
        else:
